...
CREATE FUNCTION [dbo].[CreateDependentDateRange_Local]( @sdate DATETIME, @edate DATETIME, @uid UNIQUEIDENTIFIER, @drilldown TINYINT, @calendar TINYINT, @groupBy INT=NULL )
	RETURNS @dateTable TABLE ( StartDate DATETIME, EndDate DATETIME, PeriodType VARCHAR(MAX) )
	-- This function receives dates in user time zone and returns a table also in user time zone (required after Reporting table changed to store data in local time zone)
