...
CREATE FUNCTION [dbo].[CreateDependentDateRange_UTC]( @sdate DATETIME, @edate DATETIME, @uid UNIQUEIDENTIFIER, @drilldown TINYINT, @calendar TINYINT, @groupBy INT=NULL )
...
BEGIN
	...
	--SET @sdate = '2016-04-01 06:24'
	--SET @edate = '2017-04-03 09:18'
	--SET @uid = N'7A4C7369-7E93-455E-8B66-660E91AB26C5'--N'C21039E7-58BE-4748-9A92-9AAB74AED58E'
	--SET @drilldown = 1
	--SET @calendar = 0
	--SET @groupBy = 1
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SET @curedate = CAST(DATEPART(yyyy,@cursdate) AS CHAR(4)) + '-' + CAST(DATEPART(mm,@cursdate) AS CHAR(2)) 
								+ '-' + CAST(DATEPART(dd,@cursdate) AS CHAR(2)) + ' 23:59:59'
				IF @curedate > @edate SET @curedate = @edate
				INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
				VALUES (@cursdate, @curedate, 'Day')
				
				SET @cursdate = DATEADD(ss, 1, @curedate)
			END
		END
		...
		BEGIN
			...
			BEGIN
				...
				SET @curedate = DATEADD(ss, -1, CAST(DATEPART(yyyy,@curedate) AS CHAR(4)) + '-' + CAST(DATEPART(mm,@curedate) AS CHAR(2)) 
								+ '-01 00:00')
				IF @curedate > @edate SET @curedate = @edate
				INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
				VALUES (@cursdate, @curedate, 'Month')
		...
		BEGIN
			...
			BEGIN
				...
				SET @curedate = DATEADD(ss, -1, CAST(DATEPART(yyyy,@curedate) AS CHAR(4)) + '-01-01 00:00')
				IF @curedate > @edate SET @curedate = @edate
				INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
				VALUES (@cursdate, @curedate, 'Year')
		...
		BEGIN
			...
			BEGIN
				SET @curedate = CAST(DATEPART(yyyy,@cursdate) AS CHAR(4)) + '-' + CAST(DATEPART(mm,@cursdate) AS CHAR(2)) 
								+ '-' + CAST(DATEPART(dd,@cursdate) AS CHAR(2)) + ' ' + CAST(DATEPART(hh,@cursdate) AS CHAR(2)) + ':59:59'
				IF @curedate > @edate SET @curedate = @edate
				INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
				VALUES (@cursdate, @curedate, 'Hour')
		...
		BEGIN
			...
			BEGIN
				...
				SET @curedate = CAST(DATEPART(yyyy,@cursdate) AS CHAR(4)) + '-' + CAST(DATEPART(mm,@cursdate) AS CHAR(2)) 
								+ '-' + CAST(DATEPART(dd,@cursdate) AS CHAR(2)) + ' ' + CAST(DATEPART(hh,@cursdate) AS CHAR(2)) + ':' + CAST(@mins AS CHAR(2)) + ':59'
				IF @curedate > @edate SET @curedate = @edate
				INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
				VALUES (@cursdate, @curedate, '15 Mins')
	...
	BEGIN
		...
		INSERT INTO @dateTable( Startdate , Enddate, PeriodType )
		VALUES (@cursdate, @curedate, 'Date Range')
	END
