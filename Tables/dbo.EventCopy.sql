...
GO
ALTER TABLE [dbo].[EventCopy] ADD CONSTRAINT [PK_EventCopy] PRIMARY KEY CLUSTERED  ([EventId]) WITH (FILLFACTOR=80, IGNORE_DUP_KEY=ON, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_EventCopy_ArchivedCCLatLong] ON [dbo].[EventCopy] ([Archived], [CreationCodeId], [Long], [Lat]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_Temp_EventCopyIndex] ON [dbo].[EventCopy] ([VehicleIntId]) INCLUDE ([Archived], [DriverIntId], [EventDateTime], [EventId], [Lat], [Long]) WITH (FILLFACTOR=80, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
