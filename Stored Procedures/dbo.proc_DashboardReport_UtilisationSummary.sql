...
CREATE PROCEDURE [dbo].[proc_DashboardReport_UtilisationSummary]
...
--	SET @sdate = DATEADD(dd, -7, @edate)
	...
	SELECT v.VehicleId, v.Registration, SUM(IdleTime) AS RedTime, SUM(TotalTime-IdleTime) AS GreenTime, DATEDIFF(ss, @sdate, @edate) - SUM(TotalTime) AS BlueTime
	FROM dbo.Vehicle v
	LEFT JOIN dbo.Reporting r ON r.VehicleIntId = v.VehicleIntId
