...
CREATE PROCEDURE [dbo].[proc_ReportByConfigId]
(
	@vids varchar(max),
	@dids varchar(max),
	@sdate datetime,
...
SELECT
	...
	TopGearTimeComponent,
		...
		-- Score columns
		--SweetSpotComponent + OverRevWithFuelComponent + TopGearComponent + CruiseComponent + IdleComponent + CruiseInTopGearsComponent + TopGearOverspeedComponent + CruiseOverspeedComponent
  --               + SweetSpotTimeComponent + OverRevTimeComponent + TopGearTimeComponent AS Efficiency, 
		...
		--		 + CollisionWarningHighComponent + LaneDepartureDisableComponent + LaneDepartureLeftRightComponent + FatigueComponent + DistractionComponent + SpeedGaugeComponent AS Safety,
		Efficiency,
		Safety,
		-- Additional columns with no corresponding colour	
...
FROM
	...
	FROM
		(SELECT
			...
			ISNULL(CAST(SUM(re.SweetSpotTime) AS float) / dbo.ZeroYieldNull(SUM(DrivingTime)),0) AS SweetSpotTime,
			ISNULL(CAST(SUM(re.OverRPMTime) AS float) / dbo.ZeroYieldNull(SUM(DrivingTime)),0) AS OverRevTime,
			ISNULL(CAST(SUM(re.TopGearTime) AS float) / dbo.ZeroYieldNull(SUM(DrivingTime)),0) AS TopGearTime,
		...
		FROM [US_Fleetcam_Data].[dbo].Reporting r
			INNER JOIN US_Fleetcam_Data.dbo.Vehicle v ON r.VehicleIntId = v.VehicleIntId
			LEFT JOIN US_Fleetcam_Data.dbo.FuelType f ON f.FuelTypeId = v.FuelTypeId
			INNER JOIN US_Fleetcam_Data.dbo.Driver d ON r.DriverIntId = d.DriverIntId
			LEFT JOIN US_Fleetcam_Data.dbo.ReportingABC abc ON r.VehicleIntId = abc.VehicleIntId AND r.DriverIntId = abc.DriverIntId AND r.Date = abc.Date --AND r.RouteID = abc.RouteId
			LEFT JOIN US_Fleetcam_Data.dbo.ReportingOverspeed ro ON r.VehicleIntId = ro.VehicleIntId AND r.DriverIntId = ro.DriverIntId AND r.Date = ro.Date --AND r.RouteID = ro.RouteId
			LEFT JOIN US_Fleetcam_Data.dbo.ReportingExtra re ON r.VehicleIntId = re.VehicleIntId AND r.DriverIntId = re.DriverIntId AND r.Date = re.Date
	...
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric1 ON ric1.ReportConfigurationId = @lrprtcfgid AND ric1.IndicatorId = 1
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric2 ON ric2.ReportConfigurationId = @lrprtcfgid AND ric2.IndicatorId = 2
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric3 ON ric3.ReportConfigurationId = @lrprtcfgid AND ric3.IndicatorId = 3
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric4 ON ric4.ReportConfigurationId = @lrprtcfgid AND ric4.IndicatorId = 4
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric5 ON ric5.ReportConfigurationId = @lrprtcfgid AND ric5.IndicatorId = 5
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric6 ON ric6.ReportConfigurationId = @lrprtcfgid AND ric6.IndicatorId = 6
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric7 ON ric7.ReportConfigurationId = @lrprtcfgid AND ric7.IndicatorId = 7
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric8 ON ric8.ReportConfigurationId = @lrprtcfgid AND ric8.IndicatorId = 8
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric9 ON ric9.ReportConfigurationId = @lrprtcfgid AND ric9.IndicatorId = 9
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric10 ON ric10.ReportConfigurationId = @lrprtcfgid AND ric10.IndicatorId = 10
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric11 ON ric11.ReportConfigurationId = @lrprtcfgid AND ric11.IndicatorId = 11
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric12 ON ric12.ReportConfigurationId = @lrprtcfgid AND ric12.IndicatorId = 12
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric14 ON ric14.ReportConfigurationId = @lrprtcfgid AND ric14.IndicatorId = 14
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric15 ON ric15.ReportConfigurationId = @lrprtcfgid AND ric15.IndicatorId = 15
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric16 ON ric16.ReportConfigurationId = @lrprtcfgid AND ric16.IndicatorId = 16
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric20 ON ric20.ReportConfigurationId = @lrprtcfgid AND ric20.IndicatorId = 20
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric21 ON ric21.ReportConfigurationId = @lrprtcfgid AND ric21.IndicatorId = 21
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric22 ON ric22.ReportConfigurationId = @lrprtcfgid AND ric22.IndicatorId = 22
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric23 ON ric23.ReportConfigurationId = @lrprtcfgid AND ric23.IndicatorId = 23
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric24 ON ric24.ReportConfigurationId = @lrprtcfgid AND ric24.IndicatorId = 24
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric25 ON ric25.ReportConfigurationId = @lrprtcfgid AND ric25.IndicatorId = 25
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric28 ON ric28.ReportConfigurationId = @lrprtcfgid AND ric28.IndicatorId = 28
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric29 ON ric29.ReportConfigurationId = @lrprtcfgid AND ric29.IndicatorId = 29
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric30 ON ric30.ReportConfigurationId = @lrprtcfgid AND ric30.IndicatorId = 30
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric31 ON ric31.ReportConfigurationId = @lrprtcfgid AND ric31.IndicatorId = 31
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric32 ON ric32.ReportConfigurationId = @lrprtcfgid AND ric32.IndicatorId = 32
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric33 ON ric33.ReportConfigurationId = @lrprtcfgid AND ric33.IndicatorId = 33
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric34 ON ric34.ReportConfigurationId = @lrprtcfgid AND ric34.IndicatorId = 34
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric35 ON ric35.ReportConfigurationId = @lrprtcfgid AND ric35.IndicatorId = 35
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric36 ON ric36.ReportConfigurationId = @lrprtcfgid AND ric36.IndicatorId = 36
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric37 ON ric37.ReportConfigurationId = @lrprtcfgid AND ric37.IndicatorId = 37
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric38 ON ric38.ReportConfigurationId = @lrprtcfgid AND ric38.IndicatorId = 38
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric39 ON ric39.ReportConfigurationId = @lrprtcfgid AND ric39.IndicatorId = 39
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric40 ON ric40.ReportConfigurationId = @lrprtcfgid AND ric40.IndicatorId = 40
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric41 ON ric41.ReportConfigurationId = @lrprtcfgid AND ric41.IndicatorId = 41
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric42 ON ric42.ReportConfigurationId = @lrprtcfgid AND ric42.IndicatorId = 42
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric43 ON ric43.ReportConfigurationId = @lrprtcfgid AND ric43.IndicatorId = 43
	
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric46 ON ric46.ReportConfigurationId = @lrprtcfgid AND ric46.IndicatorId = 46
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric47 ON ric47.ReportConfigurationId = @lrprtcfgid AND ric47.IndicatorId = 47
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric48 ON ric48.ReportConfigurationId = @lrprtcfgid AND ric48.IndicatorId = 48
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric49 ON ric49.ReportConfigurationId = @lrprtcfgid AND ric49.IndicatorId = 49
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric50 ON ric50.ReportConfigurationId = @lrprtcfgid AND ric50.IndicatorId = 50
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric51 ON ric51.ReportConfigurationId = @lrprtcfgid AND ric51.IndicatorId = 51
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric52 ON ric52.ReportConfigurationId = @lrprtcfgid AND ric52.IndicatorId = 52
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric53 ON ric53.ReportConfigurationId = @lrprtcfgid AND ric53.IndicatorId = 53
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric54 ON ric54.ReportConfigurationId = @lrprtcfgid AND ric54.IndicatorId = 54
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric55 ON ric55.ReportConfigurationId = @lrprtcfgid AND ric55.IndicatorId = 55
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric56 ON ric56.ReportConfigurationId = @lrprtcfgid AND ric56.IndicatorId = 56
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric57 ON ric57.ReportConfigurationId = @lrprtcfgid AND ric57.IndicatorId = 57
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric58 ON ric58.ReportConfigurationId = @lrprtcfgid AND ric58.IndicatorId = 58
	LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric59 ON ric59.ReportConfigurationId = @lrprtcfgid AND ric59.IndicatorId = 59
	...
	TopGearTimeComponent,
		...
		CASE ric51.HighLow	WHEN 1 THEN
								CASE
									...
									WHEN ric51.GYRRedMax IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) >= ric51.GYRGreenMax THEN @GreenColour
									WHEN ric51.GYRRedMax IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) >= ric51.GYRAmberMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) < ric51.GYRGreenMax THEN @YellowColour
									WHEN ric51.GYRRedMax IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) < ric51.GYRAmberMax THEN @RedColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) >= ric51.GYRRedMax THEN @GoldColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) >= ric51.GYRAmberMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) < ric51.GYRRedMax THEN @SilverColour
									WHEN ric51.GYRRedMax is NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) >= ric51.GYRGreenMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) < ric51.GYRAmberMax THEN @BronzeColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) < ric51.GYRGreenMax THEN @CopperColour
									ELSE 'Blue'
								END
							ELSE
								CASE
									WHEN ric51.GYRAmberMax = 0 AND ric51.GYRGreenMax = 0 AND ISNULL(ric51.GYRRedMax,0) = 0 THEN NULL
									WHEN ric51.GYRRedMAX IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) <= ric51.GYRAmberMax THEN @GreenColour
									WHEN ric51.GYRRedMax IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) <= ric51.GYRGreenMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) > ric51.GYRAmberMax THEN @YellowColour
									WHEN ric51.GYRRedMax IS NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) > ric51.GYRGreenMax THEN @RedColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) <= ric51.GYRRedMax THEN @GoldColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) <= ric51.GYRAmberMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) > ric51.GYRRedMax THEN @SilverColour
									WHEN ric51.GYRRedMAx IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) <= ric51.GYRGreenMax AND ROUND(CollisionWarningHigh*100, ric51.Rounding) > ric51.GYRAmberMax THEN @BronzeColour
									WHEN ric51.GYRRedMax IS NOT NULL AND ROUND(CollisionWarningHigh*100, ric51.Rounding) > ric51.GYRGreenMax THEN @CopperColour
		...
		CASE ric52.HighLow	WHEN 1 THEN
								CASE
									...
									WHEN ric52.GYRRedMax IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) >= ric52.GYRGreenMax THEN @GreenColour
									WHEN ric52.GYRRedMax IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) >= ric52.GYRAmberMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) < ric52.GYRGreenMax THEN @YellowColour
									WHEN ric52.GYRRedMax IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) < ric52.GYRAmberMax THEN @RedColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) >= ric52.GYRRedMax THEN @GoldColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) >= ric52.GYRAmberMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) < ric52.GYRRedMax THEN @SilverColour
									WHEN ric52.GYRRedMax is NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) >= ric52.GYRGreenMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) < ric52.GYRAmberMax THEN @BronzeColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) < ric52.GYRGreenMax THEN @CopperColour
									ELSE 'Blue'
								END
							ELSE
								CASE
									WHEN ric52.GYRAmberMax = 0 AND ric52.GYRGreenMax = 0 AND ISNULL(ric52.GYRRedMax,0) = 0 THEN NULL
									WHEN ric52.GYRRedMAX IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) <= ric52.GYRAmberMax THEN @GreenColour
									WHEN ric52.GYRRedMax IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) <= ric52.GYRGreenMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) > ric52.GYRAmberMax THEN @YellowColour
									WHEN ric52.GYRRedMax IS NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) > ric52.GYRGreenMax THEN @RedColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) <= ric52.GYRRedMax THEN @GoldColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) <= ric52.GYRAmberMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) > ric52.GYRRedMax THEN @SilverColour
									WHEN ric52.GYRRedMAx IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) <= ric52.GYRGreenMax AND ROUND(LaneDepartureDisable*100, ric52.Rounding) > ric52.GYRAmberMax THEN @BronzeColour
									WHEN ric52.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureDisable*100, ric52.Rounding) > ric52.GYRGreenMax THEN @CopperColour
		...
		CASE ric53.HighLow	WHEN 1 THEN
								CASE
									...
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) >= ric53.GYRGreenMax THEN @GreenColour
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) >= ric53.GYRAmberMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) < ric53.GYRGreenMax THEN @YellowColour
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) < ric53.GYRAmberMax THEN @RedColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) >= ric53.GYRRedMax THEN @GoldColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) >= ric53.GYRAmberMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) < ric53.GYRRedMax THEN @SilverColour
									WHEN ric53.GYRRedMax is NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) >= ric53.GYRGreenMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) < ric53.GYRAmberMax THEN @BronzeColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) < ric53.GYRGreenMax THEN @CopperColour
									ELSE 'Blue'
								END
							ELSE
								CASE
									WHEN ric53.GYRAmberMax = 0 AND ric53.GYRGreenMax = 0 AND ISNULL(ric53.GYRRedMax,0) = 0 THEN NULL
									WHEN ric53.GYRRedMAX IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) <= ric53.GYRAmberMax THEN @GreenColour
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) <= ric53.GYRGreenMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) > ric53.GYRAmberMax THEN @YellowColour
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) > ric53.GYRGreenMax THEN @RedColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) <= ric53.GYRRedMax THEN @GoldColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) <= ric53.GYRAmberMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) > ric53.GYRRedMax THEN @SilverColour
									WHEN ric53.GYRRedMAx IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) <= ric53.GYRGreenMax AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) > ric53.GYRAmberMax THEN @BronzeColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight*100, ric53.Rounding) > ric53.GYRGreenMax THEN @CopperColour
		...
		CASE ric57.HighLow	WHEN 1 THEN
								CASE
									...
									WHEN ric57.GYRRedMax IS NULL AND ROUND(Fatigue*100, ric57.Rounding) >= ric57.GYRGreenMax THEN @GreenColour
									WHEN ric57.GYRRedMax IS NULL AND ROUND(Fatigue*100, ric57.Rounding) >= ric57.GYRAmberMax AND ROUND(Fatigue*100, ric57.Rounding) < ric57.GYRGreenMax THEN @YellowColour
									WHEN ric57.GYRRedMax IS NULL AND ROUND(Fatigue*100, ric57.Rounding) < ric57.GYRAmberMax THEN @RedColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) >= ric57.GYRRedMax THEN @GoldColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) >= ric57.GYRAmberMax AND ROUND(Fatigue*100, ric57.Rounding) < ric57.GYRRedMax THEN @SilverColour
									WHEN ric57.GYRRedMax is NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) >= ric57.GYRGreenMax AND ROUND(Fatigue*100, ric57.Rounding) < ric57.GYRAmberMax THEN @BronzeColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) < ric57.GYRGreenMax THEN @CopperColour
									ELSE 'Blue'
								END
							ELSE
								CASE
									WHEN ric57.GYRAmberMax = 0 AND ric57.GYRGreenMax = 0 AND ISNULL(ric57.GYRRedMax,0) = 0 THEN NULL
									WHEN ric57.GYRRedMAX IS NULL AND ROUND(Fatigue*100, ric57.Rounding) <= ric57.GYRAmberMax THEN @GreenColour
									WHEN ric57.GYRRedMax IS NULL AND ROUND(Fatigue*100, ric57.Rounding) <= ric57.GYRGreenMax AND ROUND(Fatigue*100, ric57.Rounding) > ric57.GYRAmberMax THEN @YellowColour
									WHEN ric57.GYRRedMax IS NULL AND ROUND(Fatigue*100, ric57.Rounding) > ric57.GYRGreenMax THEN @RedColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) <= ric57.GYRRedMax THEN @GoldColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) <= ric57.GYRAmberMax AND ROUND(Fatigue*100, ric57.Rounding) > ric57.GYRRedMax THEN @SilverColour
									WHEN ric57.GYRRedMAx IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) <= ric57.GYRGreenMax AND ROUND(Fatigue*100, ric57.Rounding) > ric57.GYRAmberMax THEN @BronzeColour
									WHEN ric57.GYRRedMax IS NOT NULL AND ROUND(Fatigue*100, ric57.Rounding) > ric57.GYRGreenMax THEN @CopperColour
...
FROM @data dd
LEFT JOIN [US_Fleetcam_Data].[dbo].Vehicle v ON dd.VehicleId = v.VehicleId
LEFT JOIN [US_Fleetcam_Data].[dbo].Driver d ON dd.DriverId = d.DriverId
LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric1 ON ric1.ReportConfigurationId = @lrprtcfgid AND ric1.IndicatorId = 1
LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric2 ON ric2.ReportConfigurationId = @lrprtcfgid AND ric2.IndicatorId = 2
LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric3 ON ric3.ReportConfigurationId = @lrprtcfgid AND ric3.IndicatorId = 3
LEFT JOIN US_Fleetcam_Data.dbo.ReportIndicatorConfig ric4 ON ric4.ReportConfigurationId = @lrprtcfgid AND ric4.IndicatorId = 4
