...
CREATE PROCEDURE [dbo].[proc_ReportByConfigId_Vehicle]
(
	@vids VARCHAR(MAX),
	@sdate DATETIME,
	@edate DATETIME,
...
(
	Id UNIQUEIDENTIFIER,
	IntId INT,
	CO2ScaleFactor FLOAT
)
INSERT INTO @assets (Id, IntId, CO2ScaleFactor)
SELECT DISTINCT v.VehicleId, v.VehicleIntId, f.CO2ScaleFactor
FROM dbo.Vehicle v
...
FROM
	...
	FROM
		(SELECT
			...
			ISNULL((SUM(TotalFuel * ISNULL(ass.CO2ScaleFactor, 2639.1) * @co2mult)) / dbo.ZeroYieldNull(SUM(DrivingDistance + ISNULL(PTOMovingDistance,0))),0) AS Co2, --@co2mult: 1 for g/km, 1.6 for g/miles
		...
		FROM dbo.Reporting r
			INNER JOIN @assets ass ON r.VehicleIntId = ass.IntId
...
SELECT
		...
		CASE ric53.HighLow	WHEN 1 THEN
								CASE
									...
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight, ric53.Rounding) >= ric53.GYRAmberMax AND ROUND(LaneDepartureLeftRight, ric53.Rounding) < ric53.GYRGreenMax THEN @YellowColour
									...
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight, ric53.Rounding) >= ric53.GYRAmberMax AND ROUND(LaneDepartureLeftRight, ric53.Rounding) < ric53.GYRRedMax THEN @SilverColour
									WHEN ric53.GYRRedMax IS NOT NULL AND ROUND(LaneDepartureLeftRight, ric53.Rounding) >= ric53.GYRGreenMax AND ROUND(LaneDepartureLeftRight, ric53.Rounding) < ric53.GYRAmberMax THEN @BronzeColour
							...
							ELSE
								CASE
									...
									WHEN ric53.GYRRedMax IS NULL AND ROUND(LaneDepartureLeftRight, ric53.Rounding) <= ric53.GYRGreenMax AND ROUND(LaneDepartureLeftRight, ric53.Rounding) > ric53.GYRAmberMax THEN @YellowColour
