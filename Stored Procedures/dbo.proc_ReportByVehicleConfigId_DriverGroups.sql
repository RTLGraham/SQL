...
CREATE PROCEDURE [dbo].[proc_ReportByVehicleConfigId_DriverGroups]
(
	@gids VARCHAR(MAX),
	@dids varchar(max),
	@sdate datetime,
...
GROUP BY raw.GroupId, raw.VehicleId
...
FROM
	...
	FROM
		...
		FROM dbo.Reporting r
			...
			INNER JOIN @weightedData w ON dg.GroupId = w.GroupId AND w.VehicleId = v.VehicleId
			LEFT JOIN dbo.ReportingABC abc ON r.VehicleIntId = abc.VehicleIntId AND r.DriverIntId = abc.DriverIntId AND r.Date = abc.Date --AND r.RouteID = abc.RouteId
			LEFT JOIN dbo.ReportingOverspeed ro ON r.VehicleIntId = ro.VehicleIntId AND r.DriverIntId = ro.DriverIntId AND r.Date = ro.Date --AND r.RouteID = ro.RouteId
		...
		GROUP BY dg.GroupId, v.VehicleId WITH CUBE
...
-- Re-Calculate Weighted Score and Colours for Group Totals
UPDATE @data
SET Efficiency = w.WeightedEfficiency / d.TotalDrivingDistance, 
	Safety = w.WeightedSafety / d.TotalDrivingDistance
FROM @data d
INNER JOIN 
	(SELECT d.GroupId, SUM(d.Efficiency * d.TotalDrivingDistance) AS WeightedEfficiency, SUM(d.Safety * d.TotalDrivingDistance) AS WeightedSafety
	FROM @data d
	WHERE d.GroupId IS NOT NULL AND d.VehicleId IS NOT NULL
	GROUP BY d.GroupId) w ON w.GroupId = d.GroupId 
	WHERE d.GroupId IS NOT NULL AND d.VehicleId IS NULL
