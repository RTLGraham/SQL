...
CREATE PROCEDURE [dbo].[proc_Report_CombinedGroups_Vehicles]
(
	@gids varchar(max),
	@sdate datetime,
	@edate datetime,
	...
	FROM
		...
		FROM
			...
			FROM 	dbo.Reporting R
				...
				INNER JOIN dbo.Vehicle ON R.VehicleIntId = Vehicle.VehicleIntId
				INNER JOIN dbo.GroupDetail gd ON gd.EntityDataId = Vehicle.VehicleId
				INNER JOIN dbo.[Group] g ON gd.GroupId = g.GroupId
			
			WHERE R.Date BETWEEN @sdate AND @edate 
				AND g.GroupTypeId = 1 AND g.Archived = 0 AND g.IsParameter = 0
