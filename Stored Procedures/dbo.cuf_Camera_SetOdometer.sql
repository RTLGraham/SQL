...
CREATE PROCEDURE [dbo].[cuf_Camera_SetOdometer]
(
...
AS	
	...
	IF ISNULL(@Odometer, 0) > 0 AND @vid IS NOT NULL
		BEGIN	
			MERGE dbo.VehicleLatestOdometer AS target
			USING (SELECT VehicleId, CAST(@odometer AS INT) AS Odo FROM Vehicle WHERE VehicleId = @vid) AS source	
