...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[proc_CFG_ProcessIncomingConfigs_20190528] 
AS
BEGIN
	...
	-- We have a key matching an active key but the value is different but there is also an active or pending key with a matching value == new process --> new step : discontinue the non-matching key
	UPDATE h
	SET EndDate = GETUTCDATE()
	FROM @ConfigData cd
	INNER JOIN dbo.CFG_History h ON cd.IVHIntId = h.IVHIntId AND cd.KeyId = h.KeyId AND cd.KeyValue != h.KeyValue
