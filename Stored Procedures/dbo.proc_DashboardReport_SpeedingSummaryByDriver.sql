...
CREATE PROCEDURE [dbo].[proc_DashboardReport_SpeedingSummaryByDriver]
...
--	SET @sdate = DATEADD(dd, -7, @edate)
	...
	SELECT d.DriverId, dbo.FormatDriverNameByUser(d.DriverId, @uid) AS DriverName, ISNULL(SUM(ro.OverspeedDistance) / CASE WHEN SUM(r.DrivingDistance) = 0 THEN NULL ELSE SUM(r.DrivingDistance) END, 0) AS SpeedingScore,
