...
-- Description:	<Recalculates ReportingOverspeed for a single day and single vehicle. Called after a 
--				 speeding dispute has been confirmed to modify the overspeed distance. This stored proc
--				 is based on the existing proc_PopulateReportingOverspeed>
-- =======================================================================================================
CREATE PROCEDURE [dbo].[proc_RecalcReportingOverspeed] @eventid BIGINT 
AS
BEGIN

	--DECLARE @eventid BIGINT
	--SET @eventid = 201485750
	...
	-- Get the necessary data from the Event
	SELECT @vintid = VehicleIntId, @dintid = DriverIntId, @date = EventDateTime
	FROM dbo.Event
	WHERE EventId = @eventid

	SET @sdate = CAST(FLOOR(CAST(@date AS FLOAT)) AS DATETIME)
	...
	INSERT INTO @data(VehicleIntId, DriverIntId, EventSpeedingId, Speed, SpeedLimit, Distance, Lat, Long, EventDateTime, EventDateTimeReal, OverSpeedValue, OverSpeedPercent, OverSpeedHighValue, OverSpeedHighPercent, Multiplier)
		...
		WHERE e.EventDateTime BETWEEN @sdate AND @edate
		  ...
		  AND e.DriverIntId = @dintid
		GROUP BY e.VehicleIntId , e.DriverIntId, es.EventId, e.Speed , e.SpeedLimit, es.SpeedLimit ,e.OdoGPS , e.Lat, e.Long, es.ChallengeInd,
	...
	BEGIN
		...
		-- There is no overspeed distance for the vehicle/driver so delete any exising rows
		DELETE	
		FROM dbo.ReportingOverspeed
		WHERE VehicleIntId = @vintid
		  AND DriverIntId = @dintid
		  AND Date = @sdate

	END	ELSE	
	BEGIN	

		-- Update the ReportingOverspeed table using MERGE to perform an INSERT or UPDATE accordingly	
