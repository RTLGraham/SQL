...
CREATE PROC [dbo].[proc_WriteEventNewTemp]
...
BEGIN
	...
	---- write entry to EventVLE for parallel process to insert VehicleLatestEvent
	--INSERT INTO dbo.EventVLE (EventId, VehicleIntId, DriverIntId, CreationCodeId, Long, Lat, Heading, Speed, OdoGPS, OdoRoadSpeed, OdoDashboard, EventDateTime, 
	--						  DigitalIO, CustomerIntId, AnalogData0, AnalogData1, AnalogData2, AnalogData3, AnalogData4, AnalogData5, LastOperation, ProcessInd)
	...
	--         @odogps, -- OdoGPS - int
	--         @odoroadspeed, -- OdoRoadSpeed - int
	--         @ododash, -- OdoDashboard - int
	--         @eventdt, -- EventDateTime - datetime
	--         @dio, -- DigitalIO - tinyint
	--         @customerintid, -- CustomerIntId - int
	--         @analog0, -- AnalogData0 - smallint
	--         @analog1, -- AnalogData1 - smallint
	--         @analog2, -- AnalogData2 - smallint
	--         @analog3, -- AnalogData3 - smallint
	--         @analog4, -- AnalogData4 - smallint
	--         @analog5, -- AnalogData5 - smallint
	...
	BEGIN	
		...
		IF @lasteventdt IS NOT NULL AND @eventdt >= @lasteventdt AND @long <> 0 AND @lat <> 0 AND @ccid NOT IN (0,24,55,56,77,78,79,91,100,106,436,437,438,455,456,457,458)
		BEGIN
		...
		ELSE
		BEGIN
			IF @lasteventdt IS NULL AND @long <> 0 AND @lat <> 0 AND @ccid NOT IN (0,24,55,56,77,78,79,91,100,106,436,437,438,455,456,457,458) AND @rdid IS NOT NULL
			BEGIN
	...
	IF @proceed IS NULL		 		  
		...
		BEGIN	
			...
			BEGIN -- any other event so need to check whether currently in play mode
				...
				ELSE 
					SELECT @proceed = CASE WHEN @workplaymode IN (3,5) THEN 1 ELSE 0 END
				-- Additionally, if this is a Key On, Check that the PlayInd set correctly on the TripsAndStopsWorkPlay
				--IF @ccid = 4
				--BEGIN	
				--	-- Find the current trip start for this vehicle in TripsAndStops (check TripsAndStopsTemp first)
				--	SELECT TOP 1 @tripstartid = TripsAndStopsID
				...
				--	WHERE TripsAndStopsId = @tripstartid)
				--		INSERT INTO dbo.TripsAndStopsWorkPlay (TripsAndStopsId, PlayInd)
				--		VALUES  (@tripstartid, CASE WHEN @workplaymode = 3 THEN 0 ELSE 1 END)
				--END
			END		
		END		
	...
	BEGIN
		...
		IF @lasteventdt IS NOT NULL AND @eventdt >= @lasteventdt AND @long <> 0 AND @lat <> 0 AND @ccid NOT IN (0,24,55,56,77,78,79,91,100,106,436,437,438,455,456,457,458) AND @existingrows = 1
		BEGIN
		...
		ELSE
		BEGIN
			IF (@lasteventdt IS NULL OR @eventdt >= @lasteventdt) AND @long <> 0 AND @lat <> 0 AND @ccid NOT IN (0,24,55,56,77,78,79,91,100,106,436,437,438,455,456,457,458)
			BEGIN
		...
		BEGIN
			UPDATE dbo.VehicleLatestEventTemp
			SET OdoGPS = @odogps
			WHERE VehicleId = @vguid
		END	ELSE
