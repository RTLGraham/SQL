...
---- =====================================================================================================================
CREATE PROCEDURE [dbo].[proc_ProcessCameras_NoTran]
AS
...
BEGIN
	...
	BEGIN
		...
		-- Now write the corresponding Incident using Coaching Staus Id = Unknown (New)
		INSERT INTO dbo.CAM_Incident (EventId, EventDateTime, CreationCodeId, CustomerIntId, VehicleIntId, DriverIntId, CameraIntId, CoachingStatusId, ApiEventId, LastOperation, Archived)
		VALUES  (@eid, @EventDateTime, @CcId, @customerintid, @vintid, @dintid, @cameraintid, 0, @ApiEventId, GETDATE(), 0)
		...
		BEGIN
			EXEC dbo.proc_ChangeEventVideoStatus @incidentId, NULL, 1, NULL	
		END	
	...
	-- Insert the video data by matching to APIEventId
	...
	-- Now insert a trigger into TAN_TriggerEvent for all high/button videos received with status = 1 (Complete)
	...
	GROUP BY i.EventId, i.CustomerIntId, i.VehicleIntId, i.DriverIntId, e.Long, et.Long, e.Lat, et.Lat, e.Heading, et.Heading, e.Speed, et.Speed, i.EventDateTime
	...
	DECLARE vData_cursor CURSOR FAST_FORWARD FOR
		SELECT DISTINCT i.IncidentId, i.VehicleIntId, DATEADD(ss, -150, v.ApiStartTime), DATEADD(ss, 150, v.ApiEndTime)
		FROM dbo.CAM_VideoIn v
		INNER JOIN dbo.CAM_Incident i ON v.ApiEventId = i.ApiEventId AND v.Archived = 1
		WHERE i.CreationCodeId = 455

	OPEN vData_cursor
	FETCH NEXT FROM vData_cursor INTO @incidentId, @VehicleIntId, @ApiStartTime, @ApiEndTime
	WHILE @@fetch_status = 0
	BEGIN
