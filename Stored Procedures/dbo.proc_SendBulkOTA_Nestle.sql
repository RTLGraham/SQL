...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[proc_SendBulkOTA_Nestle]
...
INSERT INTO @otatable (VehicleId, OtaString, CurrentFirmware, NewFirmware)
SELECT	v.VehicleId,
		...
		+ '_' + CASE WHEN vf.Options = 'F' THEN 'GPS' ELSE vf.CanType END
		...
		+ '_' + vf.CanType
		+ CASE WHEN vf.Options IS NULL THEN '' ELSE '_' + vf.Options END
		+ CASE WHEN vf.TestVersion IS NULL THEN '' ELSE '_' + vf.TestVersion END
		+ '_' + vf.Version,
...
IF @rptonly = 1
BEGIN
	SELECT v.VehicleIntId, v.Registration, CurrentFirmware, NewFirmware, DATEDIFF(MINUTE, vle.EventDateTime, GETUTCDATE()) AS LastPollMinutes, OTAString
	FROM dbo.Vehicle v
