...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cuf_Vehicle_TrackDriverNotifications]
(
	@gids NVARCHAR(MAX),
	@vids NVARCHAR(MAX),
...
SELECT  
        ...
        vle.AnalogIoAlertTypeId AS DNStatus,
		--Last time when such DN Status was set
        MAX(dbo.[TZ_GetTime](dbo.TZ_ToUtc(dn.LastOperation, 'GMT Time', NULL), @timediff, @uid)) AS DNTime
FROM    [dbo].VehicleLatestEvent vle
        ...
        LEFT OUTER JOIN dbo.DriverNotification dn ON v.VehicleId = dn.VehicleId
                                                     AND vle.AnalogIoAlertTypeId = dn.Status
