...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[proc_WriteEventNewTempIncDriver]
...
BEGIN
	...
	SELECT @dnumber = Number FROM dbo.Driver WHERE DriverId = @did
	...
	BEGIN
		...
		SET EventId = @eid,
			...
			AnalogData4 = @analog4,
			AnalogData5 = @analog5
		WHERE DriverId = @rdid /* dbo.GetVehicleIdFromInt(@vintid) */
	END
	ELSE
	BEGIN
