...
CREATE PROCEDURE [dbo].[cuf_Vehicle_GetAccumsById]
(
...
--SET @uid = N'AE49B2CA-E1A5-4800-AD69-6218B00A6E83'
...
IF @lvids IS NULL
BEGIN
	SELECT @lvids = COALESCE(@lvids + ',', '') + CAST(VehicleId AS NVARCHAR(MAX))
	FROM 
	(
		...
		WHERE g.IsParameter = 0 AND g.GroupTypeId = 1 AND g.Archived = 0
			AND v.Archived = 0
			AND ug.Archived = 0 AND ug.UserId = @luid
	) t
END
...
FROM dbo.Vehicle
WHERE VehicleId IN (SELECT Value FROM dbo.Split(@lvids, ','))
	AND Archived = 0


/*
04/08/2020: added Start lookup for accum creation
*/
...
-- Get the start date
SELECT	
	@sdate = ISNULL(DATEADD(DAY, -1, a.ClosureDateTime), DATEADD(DAY, -1, GETUTCDATE())),
	@edate = GETUTCDATE()
FROM dbo.Accum a 
WHERE a.AccumId = @lastId
		

IF OBJECT_ID('tempdb..#ignNG') IS NOT NULL 
BEGIN 
	PRINT 'Table dropped'
	DROP TABLE #ignNG
END
...
SELECT	e.VehicleIntId,
		...
		e.Long
FROM dbo.Event e
	INNER JOIN @assets a ON e.VehicleIntId = a.IntId
WHERE e.EventDateTime BETWEEN @sdate AND @edate
	AND e.CreationCodeId = 4 --Key ON only
...
SELECT  TOP 1000 
		...
		a.VehicleIntId,
		a.DriverIntId,
		ISNULL(a.DrivingFuel, 0) + 
		ISNULL(a.ShortIdleFuel, 0) + 
		ISNULL(a.IdleFuel, 0) + 
		ISNULL(a.PTONonMovingFuel, 0) + 
		ISNULL(a.PTONonMovingFuel, 0) AS AccumulatedFuel,
		ISNULL(a.DrivingDistance, 0) +
		ISNULL(a.PTOMovingDistance, 0) AS AccumulatedDistance,
		a.IdleTime,
		a.IdleFuel,
		a.ShortIdleTime,
		a.ShortIdleFuel,
		a.CoastOutOfGearTime,
		a.CoastOutOfGearDistance,
		a.CoastOutOfGearCount,
		a.FueledOverRPMTime,
		a.FueledOverRPMDistance,
		a.FueledOverRPMFuel,
		a.ORCount AS FueledOverRPMCount,
		a.TotalVehicleDistance,
		a.TotalVehicleFuel,
		
		a.CreationCodeId,
		ign_on.EventDateTime AS CreationDateTime,
		ign_on.Lat AS CreationLat,
		ign_on.Long AS CreationLong,
		...
		a.ClosureLong
FROM    dbo.Accum a
	INNER JOIN @assets ass ON a.VehicleIntId = ass.IntId
	OUTER APPLY 
    (
		SELECT TOP 1 e.VehicleIntId, e.EventId, e.EventDateTime, e.Lat, e.Long
		FROM #ignNG e
		WHERE e.VehicleIntId = a.VehicleIntId AND e.EventDateTime < a.ClosureDateTime
		ORDER BY e.EventDateTime DESC 
    ) AS ign_on

	--Remove in prod
	INNER JOIN dbo.Vehicle v ON v.VehicleIntId = a.VehicleIntId
	INNER JOIN dbo.CustomerVehicle cv ON cv.VehicleId = v.VehicleId
	INNER JOIN dbo.Customer c ON c.CustomerId = cv.CustomerId
WHERE   a.AccumId > @llastid
ORDER BY a.AccumId
