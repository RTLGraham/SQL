...
CREATE PROCEDURE [dbo].[proc_ReportPerformance_Fleet_Vehicle]
(
...
FROM
	...
	FROM
		(SELECT		
			CASE WHEN (GROUPING(DATEPART(mm,Reporting.Date)) = 1) THEN NULL
				ELSE ISNULL(datepart(mm,Reporting.Date), NULL)
			END AS Period,
			CONVERT(CHAR(6), MAX(Reporting.Date), 112) AS ScorePeriod,
			CASE WHEN (GROUPING(GroupDetail.GroupId) = 1) THEN NULL
				ELSE ISNULL(GroupDetail.GroupId, NULL)
			END AS GroupId,
		...
		FROM 	dbo.Reporting
						...
						INNER JOIN GroupDetail ON GroupDetail.EntityDataId = v.VehicleId
						INNER JOIN dbo.UserGroup ON dbo.UserGroup.GroupId = dbo.GroupDetail.GroupId AND dbo.UserGroup.UserId = @uid
		WHERE Date BETWEEN @sdate AND @edate 
			AND dbo.UserGroup.Archived = 0
		GROUP BY DATEPART(mm,Reporting.Date), dbo.GroupDetail.GroupId WITH CUBE
