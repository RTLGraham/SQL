...
CREATE PROCEDURE [dbo].[proc_TripAnalysisAnalyse]
(
	@RequestId INT
)
AS
...
create table #RouteAnalysis (Registration varchar(50),StartGeo UNIQUEIDENTIFIER, StartDest varchar(max),StartLocation varchar(max), EndGeo UNIQUEIDENTIFIER, EndDest varchar(max),EndLocation varchar(max),StopTimeMin float, TripTimeMin float,TripDistanceKM float,StartTime smalldatetime,EndTime smalldatetime, StartID bigint,EndID bigint)

Insert into #RouteAnalysis
Select distinct v.registration,g1.GeofenceId,g1.SiteId,Upper(g1.name),g2.GeofenceId,g2.SiteId,Upper(g2.name),cast(r1.duration as float)/60,cast(r2.duration as float)/60,cast(r2.tripdistance as float) / 10 , r1.timestamp, r2.Timestamp,r1.tripsandstopsid,r2.tripsandstopsid
...
DECLARE @startgeo UNIQUEIDENTIFIER
DECLARE @startid BIGINT
Declare @start varchar(50)
Declare @startdest varchar(20)
DECLARE @endgeo UNIQUEIDENTIFIER
...
CREATE TABLE #RouteAnalysisTrips (Registration varchar(50), StartGeo UNIQUEIDENTIFIER, StartId BIGINT, StartDest varchar(20), StartLocation varchar(50), EndGeo UNIQUEIDENTIFIER, EndId BIGINT, EndDest varchar(20), EndLocation varchar(50), TripStart datetime, TripEnd datetime, SiteTime float, TripTime float, TripDistance FLOAT, Breaks FLOAT)
...
select Registration,StartGeo,StartId,StartDest,StartLocation,EndGeo,EndId,EndDest,EndLocation,TripTimeMin,TripDistanceKM,StopTimeMin,starttime,endtime
from #RouteAnalysis order by registration,starttime,endtime
Open RACursor
Fetch Next From RACursor into @creg,@cstartgeo,@cstartid,@cstartdest,@cstart,@cendgeo,@cendid,@cenddest,@cend,@cttm,@ctdm,@cstm,@cst,@cet
-- Initialise first row of data
set @reg = @creg
SET @startgeo = @cstartgeo
...
--	 OR (@end is not NULL AND @sitetime > @gap) -- we have finished at a known location for a long enough duration to record as a customer site stop
	 OR	(@cstartgeo <> @endgeo) -- starting from a new site
	 OR (@endgeo is not NULL AND @sitetime > @gap) -- we have finished at a known location for a long enough duration to record as a customer site stop
		BEGIN
			-- write the currently accumulated row
			insert into #routeanalysistrips (Registration, StartGeo, StartId, StartDest,StartLocation, EndGeo, EndId, EndDest, EndLocation, TripStart, TripEnd, SiteTime, TripTime, TripDistance, Breaks)
			values (@reg,@startgeo,@startid,@startdest,@start,@endgeo,@endid,@enddest,@end,@st,@et,@sitetime,@triptime,@tripdistance,@break)
...
--			IF @end is not NULL AND @cstm > @sitetime
			IF @endgeo is not NULL AND @cstm > @sitetime
			BEGIN
...
INNER JOIN (SELECT *, ROW_NUMBER() OVER (ORDER BY Registration, TripStart) AS 'RowNumber' FROM #routeanalysistrips) t2 ON t1.Registration = t2.Registration AND t1.EndGeo = t2.StartGeo AND t2.EndGeo = t1.StartGeo AND t2.Rownumber = t1.RowNumber + 1
GROUP BY t1.startGeo, t1.StartDest, t1.StartLocation, t1.EndDest, t1.EndLocation
) result
WHERE result.StartGeo = @siteid
...
WHERE StartGeo is not NULL
  AND EndGeo is not NULL
