...
CREATE PROCEDURE [dbo].[proc_ReportMaintenanceHealthCheck_RSNew]
    (
      @vids varchar(max),
...
--	SET @uid = N'FE90CE6B-0973-4D7B-8157-1C89CFA422F5'
	...
	SELECT	v.VehicleId,
			...
			CASE WHEN ISNULL(drv.Distance, 0) > 0 THEN 1 ELSE 0 END AS DriverIdCount,
	...
	INNER JOIN dbo.Event elast ON ev.EventId = elast.EventId
	...
	LEFT OUTER JOIN 
		(
			SELECT	v.VehicleId,
					CASE WHEN MAX(CAST(e.AnalogData0 AS INT)) - MIN(CAST(e.AnalogData0 AS INT)) > 0 THEN 1 ELSE 0 END AS Sensor01Faults,
					CASE WHEN MAX(CAST(e.AnalogData1 AS INT)) - MIN(CAST(e.AnalogData1 AS INT)) > 0 THEN 1 ELSE 0 END AS Sensor02Faults,
					CASE WHEN MAX(CAST(e.AnalogData2 AS INT)) - MIN(CAST(e.AnalogData2 AS INT)) > 0 THEN 1 ELSE 0 END AS Sensor03Faults,
					CASE WHEN MAX(CAST(e.AnalogData3 AS INT)) - MIN(CAST(e.AnalogData3 AS INT)) > 0 THEN 1 ELSE 0 END AS Sensor04Faults
			FROM dbo.Event e
				INNER JOIN dbo.Vehicle v ON e.VehicleIntId = v.VehicleIntId
				INNER JOIN dbo.VehicleLatestEvent vle ON v.VehicleId = vle.VehicleId
			WHERE v.VehicleId IN ( SELECT Value FROM dbo.Split(@vids, ',')) 
				AND e.EventDateTime BETWEEN DATEADD(DAY, @day, vle.EventDateTime) AND vle.EventDateTime
			GROUP BY v.VehicleId
	...
	LEFT OUTER JOIN
		(
			...
			FROM dbo.Reporting r
				...
				INNER JOIN dbo.VehicleLatestEvent vle ON v.VehicleId = vle.VehicleId
			WHERE v.VehicleId IN ( SELECT Value FROM dbo.Split(@vids, ',')) 
				AND r.Date BETWEEN DATEADD(DAY, @week, vle.EventDateTime) AND vle.EventDateTime
