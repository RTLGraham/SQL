...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[proc_Report_ActivityTraceByVehicle_RS]
...
--SET @vids = N'8016f50d-a2d1-49a9-bc1e-13ae27953390';
--SET @sdate = '2017-10-02 00:00';
--SET @edate = '2017-10-02 23:59';
...
SELECT v.Registration,
	 ...
	 0 AS TotalStops		 
FROM @Results r
LEFT JOIN (SELECT rt.VehicleIntID, SUM(DATEDIFF(ss, rt.StartDate, rt.EndDate)) AS DriveDuration
		 FROM @Results rt
		 WHERE rt.VehicleModeID = 1
		 GROUP BY rt.VehicleIntID) drive ON r.VehicleIntID = drive.VehicleIntID
LEFT JOIN (SELECT rt.VehicleIntID, SUM(DATEDIFF(ss, rt.StartDate, rt.EndDate)) AS IdleDuration
		 FROM @Results rt
		 WHERE rt.VehicleModeID = 2
		 GROUP BY rt.VehicleIntID) idle ON r.VehicleIntID = idle.VehicleIntID
LEFT JOIN (SELECT rt.VehicleIntID, SUM(DATEDIFF(ss, rt.StartDate, rt.EndDate)) AS KeyOnDuration
		 FROM @Results rt
		 WHERE rt.VehicleModeID = 3
		 GROUP BY rt.VehicleIntID) kon ON r.VehicleIntID = kon.VehicleIntID
LEFT JOIN (SELECT rt.VehicleIntID, SUM(DATEDIFF(ss, rt.StartDate, rt.EndDate)) AS KeyOffDuration
		 FROM @Results rt
		 WHERE rt.VehicleModeID = 4
		 GROUP BY rt.VehicleIntID) koff ON r.VehicleIntID = koff.VehicleIntID
INNER JOIN dbo.Vehicle v ON r.VehicleIntID = v.VehicleIntId
WHERE r.VehicleModeID != 0
GROUP BY v.Registration, drive.DriveDuration, idle.IdleDuration, kon.KeyOnDuration, koff.KeyOffDuration
