...
CREATE PROCEDURE [dbo].[proc_EfficiencyReportCustombyVehicleGroup]
(
	@sdate DATETIME,
	@edate DATETIME,
	@gids NVARCHAR(MAX),
...
AS
	...
	IF @configcount > 0 -- Use zero rather than one to ensure consistency when running with multiple vehicles
		EXECUTE dbo.proc_EfficiencyReportCustombyVehicleGroup_Fair
				@sdate,
				@edate,
				@gids,
				@vids,
				...
				@configid
	ELSE	
		EXECUTE dbo.proc_EfficiencyReportCustombyVehicleGroup_Standard
				@sdate,
				@edate,
				@gids,
				@vids,
