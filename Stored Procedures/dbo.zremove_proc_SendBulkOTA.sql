...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[zremove_proc_SendBulkOTA]
...
SELECT	v.VehicleId,
		...
		+ '_' + @software
		+ '";PW=00000000;ID='
		+ CHAR(ASCII('B') - ASCII('0') + ASCII(SUBSTRING(i.SerialNumber,3,1)))
		+ SUBSTRING(i.SerialNumber,2,1)
		+ SUBSTRING(i.SerialNumber,4, 50)
		+ ';*' 
		+ dbo.OTACheckSum('>STCH1' + s.FileSize + s.FileCheckSum + TFTPIPAddress + '"' + vf.Website + '_' + 'WLR' /*vf.Network*/ + '_' + ISNULL(vf.Com1, 'NON') + '_' + ISNULL(vf.Com2, 'NON') + '_' + vf.CanType + CASE WHEN vf.Options IS NULL THEN '' ELSE '_' + vf.Options END + CASE WHEN @TestVersion IS NULL THEN '' ELSE '_' + @TestVersion END + '_' + @software + '";PW=00000000;ID=' + CHAR(ASCII('B') - ASCII('0') + ASCII(SUBSTRING(i.SerialNumber,3,1))) + SUBSTRING(i.SerialNumber,2,1) + SUBSTRING(i.SerialNumber,4, 50) + ';*') 
		+ '<' END END END END
FROM dbo.Vehicle v
...
IF @rptonly = 1
BEGIN
	SELECT v.VehicleId, v.VehicleIntId, v.Registration, OTAString
	FROM dbo.Vehicle v
	INNER JOIN dbo.IVH i ON v.IVHId = i.IVHId
	INNER JOIN dbo.CustomerVehicle cv ON v.VehicleId = cv.VehicleId
