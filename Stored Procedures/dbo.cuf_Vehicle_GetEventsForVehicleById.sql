...
CREATE PROCEDURE [dbo].[cuf_Vehicle_GetEventsForVehicleById]
(
...
INNER JOIN dbo.Vehicle v WITH (NOLOCK) ON v.VehicleIntId = e.VehicleIntId
...
INNER JOIN dbo.Driver d WITH (NOLOCK) ON d.DriverIntId = e.DriverIntId
WHERE   e.VehicleIntId IN (SELECT Value FROM dbo.Split(@vIntIds, ','))
		...
		AND e.EventId > @lastId
        AND e.CreationCodeId IS NOT NULL
        AND e.CreationCodeId != 91
        AND (e.Lat != 0 AND e.Long != 0)
		AND (@workplaymode = 0 -- work/play mode is Off so select every event
